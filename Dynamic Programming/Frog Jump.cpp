...
// Solution using DP Bottom up tabular way
...
#include <bits/stdc++.h> 
int solve(int i, vector<int>&dp, vector<int>&ht)
{
    if(i==0)
        return 0;
    if(dp[i]!=-1)
    ...
    one = abs(ht[i]-ht[i-1])+solve(i-1, dp, ht);
    if(i>1)
        two = abs(ht[i]-ht[i-2])+solve(i-2, dp, ht);
