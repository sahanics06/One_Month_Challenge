...
From the head of A, it reads as [1,9,1,2,4]. From the head of B, it reads as [3,2,4]. There are 3 nodes before the intersected node in A; There are 1 node 
...
// Solution optimised T.C-O(N+M)

class Solution {
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        ListNode *l1=headA, *l2=headB;
        while(l1 != l2) // if(headA ==headB i.e same list then break and return any head) 
        {
            l1=l1->next; // Since we are checking from 2nd node
            l2=l2->next;
            if(l1==l2)
                return l1;
            if(l1==NULL)
...
// Solution 2nd optimsed
// Count nodes in list a and b. then traverse the extra node in the greater list. After that start traversing in both the list and if they are equal then return node.
...
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        ...
        while(a-b) // traversing extra nodes
        {
            l1=l1->next;
            a--;
        }
