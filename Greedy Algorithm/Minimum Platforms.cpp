...
time of one train equal to departure time of the other. At any given instance of time, same platform can not be used for both departure of a train and arrival of another 
...
Your Task:
You don't need to read input or print anything. Your task is to complete the function findPlatform() which takes the array arr[] (denoting the arrival times), array dep[] 
...
// Solution- Sort arrival and departure. Then i->arrival and j->departure; i=1; j=0; if(arrival[i]<=departure[j]) means platform is not available so 
// platform++ else j++; platform--; update ans=max(ans, platform)
